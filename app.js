...
qa('.tab').forEach(b=>{const t=b.dataset.tab;const active=t==='roadQuick'?state.road:(t==='route'?state.activeTab==='route'&&!state.road:(t==='more'?['more','fuel','emergency'].includes(state.activeTab)&&!state.road:false));b.classList.toggle('active',active)});
 ...
 q('#app').classList.toggle('road',state.road);q('#roadPanel').classList.toggle('hidden',!state.road);
...
function updateGps(){
 ...
 if(!gpsSecureContext()){
   ...
   const mode=gpsWatchId!==null?'seguimiento activo':'ubicación lista';
 ...
 distanceBox.textContent=distance===null?(coords?'Distancia pendiente hasta obtener GPS':'Sin coordenada exacta · puedes guardarla al estar físicamente en el punto'):(distance<1?`A ~${Math.round(distance*1000)} m en línea recta · radio ${Number(state.settings.proximityRadius)||350} m`:`A ~${distance.toFixed(1)} km en línea recta · radio ${Number(state.settings.proximityRadius)||350} m`);
...
qa('.tab').forEach(b=>b.addEventListener('click',()=>{const target=b.dataset.tab;if(target==='roadQuick'){state.activeTab='route';state.road=true;if(state.started&&!state.paused&&!routeCompleted()){requestWakeLock();if(state.gps?.enabled)startGpsWatch()}save();render();return}state.activeTab=target;if(target!=='route'){stopGpsWatch();releaseWakeLock();state.road=false;state.copilot=false}save();render()}));
